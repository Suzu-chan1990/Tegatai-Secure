...
if (function_exists('get_option')) {
    ...
    if ($raw !== '') {
        ...
        $client_ip = Tegatai_Logger::get_ip() ?? '';
...
class Tegatai_Firewall {
    ...
    public function run_checks() {
        // UNIVERSAL IMMUNITY: CLI, Cron & Server Loopback (Never ban localhost or server IP)
        $ip = class_exists('Tegatai_Logger') ? Tegatai_Logger::get_ip() : (isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '');
        $server_ip = isset($_SERVER['SERVER_ADDR']) ? $_SERVER['SERVER_ADDR'] : '';
        if (php_sapi_name() === 'cli' || (defined('WP_CLI') && WP_CLI)) return;
        if (in_array($ip, ['127.0.0.1', '::1', $server_ip])) return;
    ...
    private function run_waf() { 
        ...
        if (strpos($_SERVER['REQUEST_URI'], '/secret-backup-db/') !== false) {
            ...
            $k = 'teg_404_'.md5(Tegatai_Logger::get_ip());
    ...
    private function check_whitelist($l) { return in_array(Tegatai_Logger::get_ip(), array_map('trim', explode("\n", $l))); }
    ...
    private function check_blacklist($l) { if(in_array(Tegatai_Logger::get_ip(), array_map('trim',explode("\n",$l)))) wp_die('Banned','403',['response'=>403]); }
    private function check_rate_limit() { 
        ...
        $k = 'teg_rl_' . md5(Tegatai_Logger::get_ip() . ($is_api ? '_api' : '')); 
