...
/* TEGATAI_USER_HISTORY_UNIFICATION_V1 applied */
class Tegatai_UserHistory {
    ...
    public function log_login($user_login, $user) {
        $ip = class_exists('Tegatai_Logger') && method_exists('Tegatai_Logger', 'get_ip') ? Tegatai_Logger::get_ip() : ($_SERVER['REMOTE_ADDR'] ?? '');
        $role = !empty($user->roles) ? reset($user->roles) : 'none';
        
        // 1. Legacy Option (Für Abwärtskompatibilität und Dashboard Widgets)
        ...
        // 2. UNIFIED LOGGING: Logins ins zentrale Terminal und Log-System pushen
        if (class_exists('Tegatai_Logger')) {
            Tegatai_Logger::log('LOGIN', "User logged in: $user_login (Role: $role)", $ip);
        }
    }
    ...
    public function clear_history() {
        ...
        wp_safe_redirect(admin_url('admin.php?page=tegatai-secure&tab=sessions&msg=hist_cleared')); 
        exit;
    }

    public static function get_history() { 
        return get_option('tegatai_login_history', []); 
    }
}
